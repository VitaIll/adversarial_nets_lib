...
class AdversarialEstimator:
    ...
    def callibrate(
        ...
        optimizer_params : dict
            Parameters for :func:`optuna.create_study` (except
            ``direction``). May include ``n_trials`` to specify the number of
            optimization trials.
        metric_name : str
            Calibration metric passed to :func:`evaluate_discriminator`.
        k : int, optional
        ...
        direction : {"minimize", "maximize"}, optional
            ...
            Default is "minimize" since calibration metrics are typically
            losses.
        """

        n_trials = optimizer_params.pop("n_trials", 50)
        optimizer_params.pop("direction", None)
